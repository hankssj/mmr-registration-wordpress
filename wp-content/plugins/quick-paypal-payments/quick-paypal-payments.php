...
function qpp_display_form($values, $errors, $id, $attr = '') {
    ...
    if (count($errors) > 0) {
		...
		"<".$hd." class='qpp-header' id='qpp_reload' style='color:".$style['error-colour'].";'>" . $error['errortitle'] . "</".$hd.">        
        ...
        $arr = array(
            ...
            'quantity',
            'use_stock',
            'answer','quantity','email','firstname','lastname','address1','address2','city','state','zip','country','night_phone_b'
        );
	...
	*/
	...
	foreach (explode( ',',$qpp['sort']) as $name) {
        switch ( $name ) {
            ...
            if (!$values['setref']) {
                ...
                $content .= '<p>
                <input type="text" '.$required.$errors['reference'].' id="reference" name="reference" value="' . $values['reference'] . '" rel="'. $values['reference'] . '" onfocus="qppclear(this, \'' . $values['reference'] . '\')" onblur="qpprecall(this, \'' . $values['reference'] . '\')"/></p>';
            } else {
                if ($values['combine']) {
                    $checked = 'checked';
            ...
            if ($qpp['use_slider'] && !$values['combine']) {
                ...
                if (!$values['combine']) {
                    if (!$values['setpay']){
                        ...
                        $content .= '<p>
                        <input type="text" rel="'.$values['amount'].'" '.$required.$errors['amount'].' id="qppamount'.$t.'" label="Amount" name="amount" value="' . $values['amount'] . '" onfocus="qppclear(this, \'' . $values['amount'] . '\')" onblur="qpprecall(this, \'' . $values['amount'] . '\' )" />
                        </p>';
                    } else {
    ...
    if ($style['submit-button']) {
        $content .= '<p class="submit"><input type="image" id="submitimage" value="' . $caption . '" src="'.$style['submit-button'].'" name="qppsubmit'.$id.'" /></p>';
    } else {
    ...
    $content .= '</form>'."\r\t";
	$content .= '<script type="text/javascript"> qpp_containers.push(jQuery(document.getElementById("frmPayment'.(($id)? $id:'default').'")).find("p.submit input").get(0)); </script>';
...
function qpp_process_form($values,$id) {
    ...
    $check = (float) qpp_format_amount($currency[$id],$qpp,$values['amount']);
    ...
    if ($qpp['userecurring']) {
        ...
        if ((isset($send['combine']) && !$send['combine']) || !isset($send['combine'])) {
            if ($qpp['useprocess']) {
                ...
                $content .='<input type="hidden" name="handling" value="0.00">';
            }
            if ($qpp['usepostage']) {
                $content .='<input type="hidden" name="shipping" value="' . $packing . '">';
            } else {
                $content .='<input type="hidden" name="shipping" value="0.00">';
            }
        }
    }
    ...
    if (isset($send['mailchimpuser']) && $send['mailchimpuser'] && $values['email'])
        $content .= qpp_mailchimp($values,$send);
...
function qpp_start_transaction(&$paypal,$currency,$qpp,$amount) {

	/*
		Use GET Variables
	*/
	$amount = (float) preg_replace('/([^0-9.,])/','',(($amount)? $amount:0));
	$name = (string) $_POST['reference'];
	...
	*/
	...
	/*
		Build Note
	*/
	if (isset($_POST['yourmessage'])) $order->setAttribute('NOTETEXT',$_POST['yourmessage']);
	...
	*/
	...
	foreach ($_POST as $k => $v) {
		if (array_key_exists($k,$a_vars)) {
			...
			switch ($prop) {
				case 'CALCULATE':
					if ($k == 'firstname') $fname = $v;
					else $lname = $v;
				break;
				default:
					$order->setAttribute($prop,$v);
				break;
			}
		}
	}
